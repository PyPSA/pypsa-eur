...
scenarios_steel = ["base_eu_regain", "policy_eu_regain","base_eu_maintain",  "policy_eu_maintain", "base_eu_deindustrial","policy_eu_deindustrial"]
...
def plot_steel_scenarios(scenarios):
    ...
    fig, axes = plt.subplots(2, len(scenarios)//2, figsize=(20, 12), sharex=True, sharey=True)
    ...
    plt.tight_layout()
    plt.savefig("./graphs/steel_production2.png")
    plt.show()
...
def plot_cement_scenarios(scenarios):
    ...
    for scenario in scenarios:
        ...
        summed_prod_cement_not_captured.index = [country_names.get(code, code) for code in summed_prod_cement_not_captured.index]
        summed_prod_cement_captured.index = [country_names.get(code, code) for code in summed_prod_cement_captured.index]
        ...
        for country in all_countries:
            ...
            country_totals[full_name] = country_totals.get(full_name, 0) + summed_prod_cement.get(country, 0)
    ...
    for i, scenario in enumerate(scenarios):
        summed_prod_cement_not_captured = all_scenario_data[scenario]["summed_prod_cement_not_captured"].reindex(relevant_countries, fill_value=0)
        summed_prod_cement_captured = all_scenario_data[scenario]["summed_prod_cement_captured"].reindex(relevant_countries, fill_value=0)
        ...
        # Plot data
        ax.bar(sorted_countries, summed_prod_cement_not_captured.values, color="gray", label="Cement Not Captured")
        ax.bar(sorted_countries, summed_prod_cement_captured.values, color="orange", label="Cement Captured", bottom=summed_prod_cement_not_captured.values)
    ...
    plt.tight_layout()
    plt.savefig("./graphs/cement_production2.png")
    plt.show()
...
def plot_ammonia_scenarios(scenarios):
    ...
    for i, scenario in enumerate(scenarios):
        ...
        # Plot in subplot
        ax = axes[i]
        ax.barh(sorted_countries, prod_nh3_grey.values, color="#552C2D", label="Haber-Bosch with grey H2")
...
def plot_ammonia_scenarios2(scenarios):
    country_totals = {}  # Track total production per country
    all_scenario_data = {}  # Store all relevant data for each scenario

    # First pass: Collect data across all scenarios
    for scenario in scenarios:
        n = pypsa.Network(f"C:/Users/alice/Desktop/CMCC/pypsa-adb-industry/results/{scenario}/networks/base_s_39___2040.nc")
        timestep = n.snapshot_weightings.iloc[0, 0]
        ...
        if not np.allclose(summed_prod_nh3_green + summed_prod_nh3_grey, summed_prod_nh3, atol=1e-6):
            print(f"Error in scenario {scenario}: Ammonia data inconsistency detected.")

        # Map country codes to names
        summed_prod_nh3_green.index = [country_names.get(code, code) for code in summed_prod_nh3_green.index]
        summed_prod_nh3_grey.index = [country_names.get(code, code) for code in summed_prod_nh3_grey.index]
        ...
        for country in all_countries:
            full_name = country_names.get(country, country)
            country_totals[full_name] = country_totals.get(full_name, 0) + summed_prod_nh3.get(country, 0)
    ...
    for i, scenario in enumerate(scenarios):
        summed_prod_nh3_green = all_scenario_data[scenario]["summed_prod_nh3_green"].reindex(relevant_countries, fill_value=0)
        summed_prod_nh3_grey = all_scenario_data[scenario]["summed_prod_nh3_grey"].reindex(relevant_countries, fill_value=0)
        ...
        # Plot data
        ax.bar(sorted_countries, summed_prod_nh3_grey.values, color="#552C2D", label="Grey H2 Ammonia")
        ax.bar(sorted_countries, summed_prod_nh3_green.values, color="green", label="Green H2 Ammonia", bottom=summed_prod_nh3_grey.values)
    ...
    plt.tight_layout()
    plt.savefig("./graphs/ammonia_production2.png")
    plt.show()
...
def plot_methanol_scenarios(scenarios):
    ...
    for scenario in scenarios:
        ...
        prod_methanol = -n.links_t.p1.filter(like="methanolisation", axis=1).sum() * timestep
        ...
        if not np.allclose(prod_methanol_green + prod_methanol_grey, prod_methanol, atol=1e-6):
            print(f"Error in scenario {scenario}: The sum of Grey H2 and Green H2 methanol does not match the original methanol production.")
        ...
        for country in all_countries:
            if country not in country_totals:
                country_totals[country] = 0
            country_totals[country] += prod_methanol_green.get(country, 0) + prod_methanol_grey.get(country, 0)
    ...
    for i, scenario in enumerate(scenarios):
        prod_methanol_green = all_scenario_data[scenario]["prod_methanol_green"].reindex(relevant_countries, fill_value=0)
        prod_methanol_grey = all_scenario_data[scenario]["prod_methanol_grey"].reindex(relevant_countries, fill_value=0)
        ...
        ax = axes[i]
        ax.bar(sorted_countries, prod_methanol_grey.values, color="brown", label="Methanolisation with grey H2")
        ax.bar(sorted_countries, prod_methanol_green.values, color="green", label="Methanolisation with green H2", left=prod_methanol_grey.values)
...
def plot_methanol_scenarios2(scenarios):
    country_totals = {}
    all_scenario_data = {}
    ...
    for scenario in scenarios:
        ...
        prod_methanol_green.index = [country_names.get(code, code) for code in prod_methanol_green.index]
        prod_methanol_grey.index = [country_names.get(code, code) for code in prod_methanol_grey.index]
    ...
    for i, scenario in enumerate(scenarios):
        methanol_green = all_scenario_data[scenario]["methanol_green"].reindex(relevant_countries, fill_value=0)
        methanol_grey = all_scenario_data[scenario]["methanol_grey"].reindex(relevant_countries, fill_value=0)
        ...
        ax.bar(sorted_countries, methanol_grey.values, color="grey", label="Methanol with grey H2")
        ax.bar(sorted_countries, methanol_green.values, color="green", label="Methanol with green H2", bottom=methanol_grey.values)
    ...
    plt.tight_layout()
    plt.savefig("./graphs/methanol_production2.png")
    plt.show()
...
def plot_capacities2(csv_file):
    ...
    remaining_columns = [col for col in df_grouped.columns if col not in stacked_columns]
    
    # Create the figure
    fig, axes = plt.subplots(nrows=len(remaining_columns) + 1, figsize=(10, 5 * (len(remaining_columns) + 1)))
    
    # Plot the stacked bar for Integrated Steelworks and EAF
    df_filtered.plot(kind="bar", stacked=True, ax=axes[0], width=1, color=["steelblue", "darkorange"], edgecolor="black")
    ...
    for i, column in enumerate(remaining_columns, start=1):
        ...
        df_sorted.plot(kind="bar", ax=axes[i], color="skyblue", edgecolor="black", width=1)
...
def plot_hvc_scenarios2(scenarios):
    country_totals = {}  # Track total production per country
    all_scenario_data = {}  # Store all relevant data for each scenario
    ...
    for scenario in scenarios:
        ...
        prod_naphtha_cracker = -n.links_t.p1.filter(like="naphtha steam cracker", axis=1).sum() * timestep
        prod_naphtha_cracker.index = prod_naphtha_cracker.index.str[:2]  # Keep only country code
        prod_naphtha_cracker = prod_naphtha_cracker[prod_naphtha_cracker > 1e-10]
        prod_naphtha_cracker = prod_naphtha_cracker.groupby(prod_naphtha_cracker.index).sum()

        prod_methanol_hvc = -n.links_t.p1.filter(like="methanol-to-hvc", axis=1).sum() * timestep
        ...
        # Extract emissions data
        oil_prod_biomass = -n.links_t.p1.filter(like="biomass to liquid", axis=1).sum() * timestep
        oil_prod_biomass.index = oil_prod_biomass.index.str[:2]
        oil_prod_biomass = oil_prod_biomass[oil_prod_biomass > 0].groupby(oil_prod_biomass.index).sum()
        ...
        # Map country codes to full country names
        prod_naphtha_bio.index = [country_names.get(code, code) for code in prod_naphtha_bio.index]
        prod_naphtha_grey.index = [country_names.get(code, code) for code in prod_naphtha_grey.index]
        prod_methanol_hvc_green.index = [country_names.get(code, code) for code in prod_methanol_hvc_green.index]
        prod_methanol_hvc_grey.index = [country_names.get(code, code) for code in prod_methanol_hvc_grey.index]
        ...
        for country in all_countries:
            full_name = country_names.get(country, country)
            country_totals[full_name] = country_totals.get(full_name, 0) + (
                prod_naphtha_grey.get(country, 0) + prod_naphtha_bio.get(country, 0) +
                prod_methanol_hvc_green.get(country, 0) + prod_methanol_hvc_grey.get(country, 0)
            )
    ...
    for i, scenario in enumerate(scenarios):
        hvc_fossil_naphtha = all_scenario_data[scenario]["hvc_fossil_naphtha"].reindex(relevant_countries, fill_value=0)
        hvc_bio_naphtha = all_scenario_data[scenario]["hvc_bio_naphtha"].reindex(relevant_countries, fill_value=0)
        hvc_methanol_grey = all_scenario_data[scenario]["hvc_methanol_grey"].reindex(relevant_countries, fill_value=0)
        hvc_methanol_green = all_scenario_data[scenario]["hvc_methanol_green"].reindex(relevant_countries, fill_value=0)

        total_production = hvc_fossil_naphtha + hvc_bio_naphtha + hvc_methanol_grey + hvc_methanol_green
        sorted_countries = total_production.sort_values(ascending=False).index
        ...
        ax.bar(sorted_countries, hvc_fossil_naphtha.values, color="black", label="Fossil Naphtha Cracker")
        ax.bar(sorted_countries, hvc_bio_naphtha.values, color="brown", label="Bio Naphtha Cracker", bottom=hvc_fossil_naphtha.values)
        ax.bar(sorted_countries, hvc_methanol_grey.values, color="grey", label="Methanol Cracker with grey H2", bottom=hvc_fossil_naphtha.values + hvc_bio_naphtha.values)
        ax.bar(sorted_countries, hvc_methanol_green.values, color="green", label="Methanol Cracker with green H2", bottom=hvc_fossil_naphtha.values + hvc_bio_naphtha.values + hvc_methanol_grey.values)
    ...
    plt.tight_layout()
    plt.savefig("./graphs/hvc_production2.png")
    plt.show()
...
plot_steel_scenarios(scenarios_steel)
plot_cement_scenarios(scenarios_steel)
plot_ammonia_scenarios(scenarios)
plot_ammonia_scenarios2(scenarios_steel)

plot_hvc_scenarios2(scenarios_steel)
plot_methanol_scenarios2(scenarios_steel)
plot_capacities2("capacities_s_39.csv")
