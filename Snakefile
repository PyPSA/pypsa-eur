...
rule cluster_all_networks:
    input:
        expand("networks/elec_s{simpl}_{clusters}.nc", **config["scenario"]),


rule extra_components_all_networks:
    input:
        expand("networks/elec_s{simpl}_{clusters}_ec.nc", **config["scenario"]),


rule prepare_all_networks:
    input:
        expand(
            "networks/elec_s{simpl}_{clusters}_ec_l{ll}_{opts}.nc", **config["scenario"]
        ),
...
if config["enable"].get("retrieve_cutout", True):
    ...
    rule retrieve_cutout:
        input:
            HTTP.remote("zenodo.org/record/4709858/files/{cutout}.nc", keep_local=True),
        output:
            "cutouts/{cutout}.nc",
        shell:
            "mv {input} {output}"
...
if config["enable"].get("retrieve_natura_raster", True):
    ...
    rule retrieve_natura_raster:
        input:
            HTTP.remote("zenodo.org/record/4706686/files/natura.tiff", keep_local=True),
        output:
            "resources/natura.tiff",
        shell:
            "mv {input} {output}"
...
rule build_renewable_profiles:
    input:
        ...
        cutout=(
            lambda w: "cutouts/" + config["renewable"][w.technology]["cutout"] + ".nc"
        ),
    output:
...
rule solve_network:
    ...
    log:
        ...
        memory=(
            "logs/solve_network/elec_s{simpl}_{clusters}_ec_l{ll}_{opts}_memory.log"
        ),
    benchmark:
...
def input_plot_p_nom_max(w):
    return [
        (
            "networks/elec_s{simpl}{maybe_cluster}.nc".format(
...
rule plot_p_nom_max:
    ...
    output:
        "results/plots/elec_s{simpl}_cum_p_nom_max_{clusts}_{techs}_{country}.{ext}",
    log:
        "logs/plot_p_nom_max/elec_s{simpl}_{clusts}_{techs}_{country}_{ext}.log",
    script:
