...
def plot_steel_scenarios(scenarios):
    ...
    for scenario in scenarios:
        ...
        file_path = os.path.join(
            parent_dir, "results_8h", scenario, "networks", "base_s_39___2030.nc"
        )
        ...
        for country in all_countries:
            ...
            country_totals[country] += (
                summed_p_nom_ch4_eaf.get(country, 0)
                + summed_p_nom_grey_h2_eaf.get(country, 0)
                + summed_p_nom_green_h2_eaf.get(country, 0)
...
def plot_steel_scenarios_pathways(years, scenarios):
    ...
    for scenario in scenarios:
        for year in years:
            ...
            p_nom_eaf = (
                -n.links_t.p1.filter(like="EAF").sum() * timestep / 1e3
            )  # Convert to Mt
            ...
            share_green = share_green_h2(n).reindex(
                summed_p_nom_eaf.index, fill_value=0
            )
            ...
            for country in summed_p_nom_eaf.index.union(summed_p_nom_bof.index):
                country_totals[country] = (
                    country_totals.get(country, 0)
                    + summed_p_nom_eaf.get(country, 0)
                    + summed_p_nom_bof.get(country, 0)
                )
    ...
    for row, scenario in enumerate(scenarios):
        for col, year in enumerate(years):
            ...
            sorted_countries = [
                relevant_countries[code]
                for code in total_production.sort_values(ascending=False).index
            ]

            # Plot bars
            bottom = np.zeros(len(sorted_countries))
            for label, color in zip(
                ["bof", "ch4_eaf", "grey_h2_eaf", "green_h2_eaf"],
                ["black", "#552C2D", "gray", "green"],
            ):
                ax.bar(
                    sorted_countries,
                    data[label].values,
...
scenario_labels = {"base_eu_regain": "Baseline", "policy_eu_regain": "Policy"}
...
line_styles = {
    "base_eu_regain": "-",
    "policy_eu_regain": "-",
}  # Solid for baseline, dashed for policy
...
aloads_t_2030 = (
    n.loads_t.p.loc[:, n.loads_t.p.columns.str.contains("steel")].sum() * timestep
)
...
parent_dir = os.path.dirname(cwd)
file_path = os.path.join(
    parent_dir, "results_12h", scenario, "networks", f"base_s_39___{year}.nc"
)
n = pypsa.Network(file_path)
timestep = n.snapshot_weightings.iloc[0, 0]
