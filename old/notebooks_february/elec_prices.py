...
scenarios = ["base_reg_regain", "base_reg_maintain", "base_reg_deindustrial", "policy_reg_regain","policy_reg_maintain", "policy_reg_deindustrial"]
...
# Electricity
elec_total_costs = total_costs[total_costs.columns[total_costs.columns.str.endswith(" 0") | total_costs.columns.str.contains("electricity")]]
elec_loads_w_mprice = loads_w_mprice[loads_w_mprice.columns[loads_w_mprice.columns.str.endswith(" 0") | loads_w_mprice.columns.str.contains("electricity")]]
...
scenarios = ["base_reg_regain","base_eu_regain",
             "base_reg_deindustrial", "base_eu_deindustrial",
...
price_data = {country: pd.DataFrame(index=scenarios, columns= years) for country in country_names.values()}
...
for idx, country in enumerate(country_names.values()):
    ...
    for i, scenario in enumerate(scenarios):
        ax.plot(years, price_data[country].loc[scenario], marker="o", linestyle="-", label=scenario)
...
for scenario in scenarios:
    for year in years:
        ...
        # Energy
        ener_total_costs = total_costs[total_costs.columns[~total_costs.columns.str.endswith(" 0") & ~total_costs.columns.str.contains("electricity") & ~total_costs.columns.str.match(r'^[a-z]')]]
        ener_loads_w_mprice = loads_w_mprice[loads_w_mprice.columns[~loads_w_mprice.columns.str.endswith(" 0") & ~loads_w_mprice.columns.str.contains("electricity") & ~total_costs.columns.str.match(r'^[a-z]')]]
...
# %%
colors = {
    "base_eu_regain": "#4F5050", "base_eu_deindustrial": "#85877C", "base_reg_deindustrial": "#B0B2A1", "base_reg_regain": "grey",
    "policy_eu_regain": "#5D8850", "policy_eu_deindustrial": "#95BF74", "policy_reg_deindustrial": "#C5DEB1", "policy_reg_regain": "green"
}
