...
def plot_commodity_map(n, regions, year, commodity, ax=None):
    ...
    commodity_prod_index = n.links[
        n.links["bus1"].str.contains(commodity, case=False, na=False)
        & ~n.links["bus1"].str.contains("heat", case=False, na=False)
    ].index
    ...
    regions[commodity] = (
        commodity_prod_df[f"{commodity}_prod"].groupby(level=0).sum().div(1e3)
        * timestep
    )
    ...
    ax.set_facecolor("white")
    ax.set_title(
        f"{commodity.capitalize()} Production {year}", fontsize=14, loc="center"
    )
