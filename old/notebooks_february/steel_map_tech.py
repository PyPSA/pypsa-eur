...
def plot_steel_map(n, regions, year, i, ax=None):
    ...
    steel_prod_tech = -n.links_t.p1.loc[
        :, steel_prod_index
    ].sum()  # *timestep/1e3 #Mtsteel
    ...
    steel_prod_grey_h2_eaf = (
        steel_prod_eaf * share_h2 * (1 - share_green)
    )  # EAF with Grey H2
    steel_prod_green_h2_eaf = (
        steel_prod_eaf * share_h2 * share_green
    )  # EAF with Green H2
...
with open(
    r"C:\Users\Dibella\Desktop\CMCC\pypsa-adb-industry\results_8h\base_eu_regain\configs\config.base_s_39___2030.yaml"
) as config_file:
