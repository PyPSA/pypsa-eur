"""
Created on Fri Dec  6 14:57:16 2024
...
def plot_pie_map(
    n, ammonia_prod, ammonia_tech, regions, year, i, j, piecolors, ax=None
):
    assign_location(n)
...
for j, scenario in enumerate(scenarios):
    for i, year in enumerate(years):
        ammonia_prod, ammonia_tech = ammonia_pie_preprocessing(
            excel_dir, scenario, config, year
        )

        ax = axes[j, i]
        plot_pie_map(
            n, ammonia_prod, ammonia_tech, regions, year, i, j, piecolors, ax=ax
        )
        ax.axis("off")
