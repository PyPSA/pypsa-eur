...
n = pypsa.Network(
    "../results_3h/baseline_eu_dem/postnetworks/base_s_39_lvopt___2030.nc"
)
...
for scenario in scenarios:
    ...
    with pd.ExcelWriter("excels_3h/" + output_filename, engine="xlsxwriter") as writer:
        ...
        for year in years:
            n = pypsa.Network(
                f"../results/{scenario}/postnetworks/base_s_39_lvopt___{year}.nc"
            )
            timestep = n.snapshot_weightings.iloc[0, 0]

            # EAF capacity
            eaf_efficiency = n.links.efficiency.loc[
                n.links.index.str.contains("EAF", case=False)
            ].iloc[0]
            eaf_capacity = (
                n.links.p_nom_opt[n.links.index.str.contains("EAF")]
                * 8760
                * eaf_efficiency
            )  # kton steel / year
            eaf_capacity = country_values(eaf_capacity)
            eaf_capacity_df[year] = eaf_capacity.reindex(eaf_capacity_df.index).fillna(
                0
            )
            ...
            bof_capacity = (
                n.links.p_nom_opt[n.links.index.str.contains("BOF")]
                * 8760
                * bof_efficiency
            )  # kton steel / year
            bof_capacity = country_values(bof_capacity)
            bof_capacity_df[year] = bof_capacity.reindex(bof_capacity_df.index).fillna(
                0
            )

            # H2 EAF
            h2_eaf = (
                n.links_t.p0.filter(like="H2 to DRI", axis=1).sum() * timestep
            )  # MWh H2 / year
            ...
            # NG EAF
            ng_eaf = (
                n.links_t.p0.filter(like="CH4 to DRI", axis=1).sum() * timestep
            )  # MWh CH4 / year
            ...
            # Steel TGR capacity
            tgr_capacity = (
                n.links.p_nom_opt[n.links.index.str.contains("steel bof CC")] * 8760
            )  # kton steel / year
            tgr_capacity = country_values(tgr_capacity)
            tgr_capacity_df[year] = tgr_capacity.reindex(tgr_capacity_df.index).fillna(
                0
            )

            # EAF production
            eaf_prod = (
                -n.links_t.p1.filter(like="EAF", axis=1).sum() * timestep
            )  # kt steel / year
            ...
            steel_price_df[year] = column_averages.reindex(steel_price_df.index).fillna(
                0
            )
            ...
            elec_prices_df[year] = weighted_average.reindex(
                elec_prices_df.index
            ).fillna(0)

            # Ammonia production
            ammonia_prod = (
                -n.links_t.p1.filter(like="Haber", axis=1).sum() * timestep
            )  # kt NH3 / year
            ammonia_prod = country_values(ammonia_prod)
            ammonia_prod_df[year] = ammonia_prod.reindex(ammonia_prod_df.index).fillna(
                0
            )

            # Ammonia load
            ammonia_load = (
                n.loads_t.p.filter(like="NH3", axis=1).sum() * timestep
            )  # MWh NH3 / year
            ammonia_load = country_values(ammonia_load)
            ammonia_load_df[year] = ammonia_load.reindex(ammonia_load_df.index).fillna(
                0
            )
            ...
            # Electrolysis
            elec_h2prod = (
                -n.links_t.p1.filter(like="Electrolysis", axis=1).sum() * timestep
            )
            ...
            smrcc_h2prod_df[year] = smrcc_h2prod.reindex(smrcc_h2prod_df.index).fillna(
                0
            )
            ...
            smr_h2prod = (
                -n.links_t.p1.loc[
                    :,
                    n.links_t.p1.columns.str.contains("SMR")
                    & ~n.links_t.p1.columns.str.contains("SMR CC"),
                ].sum()
                * timestep
            )
            ...
            # Ammonia
            nh3crack_h2prod = (
                -n.links_t.p1.filter(like="ammonia cracker", axis=1).sum() * timestep
            )
            nh3crack_h2prod = country_values(nh3crack_h2prod)
            nh3crack_h2prod_df[year] = nh3crack_h2prod.reindex(
                nh3crack_h2prod_df.index
            ).fillna(0)
            ...
            methanolisation_df[year] = methanol_prod.reindex(
                methanolisation_df.index
            ).fillna(0)

            methanol_load = (
                n.loads_t.p.filter(like="methanol", axis=1).sum() * timestep
            )  # MWh methanol per year per node
            methanol_load = country_values(methanol_load)
            methanol_load_df[year] = methanol_load.reindex(
                methanol_load_df.index
            ).fillna(0)

            # Emissions
            dri_emis = (
                -n.links_t.p1.filter(like="dri process", axis=1).sum() * timestep / 1e3
            )  # from t to kt per year
            dri_emis = country_values(dri_emis)
            bof_emis = (
                -n.links_t.p1.filter(like="bof process", axis=1).sum() * timestep / 1e3
            )  # from t to kt per year
            bof_emis = country_values(bof_emis)
            bofcc_emis = (
                -n.links_t.p1.filter(like="bof CC", axis=1).sum() * timestep / 1e3
            )  # from t to kt per year
            bofcc_emis = country_values(bofcc_emis)
            steel_proc_emis = dri_emis + bof_emis + bofcc_emis
            steel_proc_emis_df[year] = steel_proc_emis.reindex(
                steel_proc_emis_df.index
            ).fillna(0)

            bofcc_emis_capt = (
                -n.links_t.p2.filter(like="bof CC", axis=1).sum() * timestep / 1e3
            )  # from t to kt per year
            bofcc_emis_capt = country_values(bofcc_emis_capt)
            bofcc_emis_capt_df[year] = bofcc_emis_capt.reindex(
                bofcc_emis_capt_df.index
            ).fillna(0)

            bofcc_emis_tgr = (
                n.links_t.p0.filter(like="bof CC", axis=1).sum() * timestep / 1e3
            )  # from t to kt per year
            ...
            cement_emis = (
                -n.links_t.p1.filter(like="cement process", axis=1).sum()
                * timestep
                / 1e3
            )  # from t to kt per year
            cement_emis = country_values(cement_emis)
            cementcc_emis = (
                -n.links_t.p1.filter(like="cement CC", axis=1).sum() * timestep / 1e3
            )  # from t to kt per year
            cementcc_emis = country_values(cementcc_emis)
            cement_proc_emis = cement_emis + cementcc_emis
            cement_proc_emis_df[year] = cement_proc_emis.reindex(
                cement_proc_emis_df.index
            ).fillna(0)
