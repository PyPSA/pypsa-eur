...
n = pypsa.Network(
    "../results/climate_policy_eu_dem/postnetworks/base_s_39_lvopt___2050.nc"
)
...
cement_load = (
    n.loads_t.p.filter(like="cement", axis=1).sum() * timestep
)  # kton cement per year per node
cement_load = country_values(cement_load)
cement_supply = (
    -n.links_t.p1.filter(like="Cement", axis=1).sum() * timestep
)  # kton cement per year per node
cement_supply = country_values(cement_supply)

# Check if energy consumption and emission balances are respected
cement_links_p0 = (
    n.links_t.p0.filter(like="Cement", axis=1).sum() * timestep
)  # kton limestone per year per node
cement_links_p0 = country_values(cement_links_p0)
cement_links_p2 = (
    n.links_t.p2.filter(like="Cement", axis=1).sum() * timestep
)  # MWh industry heat per year per node
cement_links_p2 = country_values(cement_links_p2)
cement_links_p3 = (
    n.links_t.p3.filter(like="Cement", axis=1).sum() * timestep
)  # tCO2 per yaer per node
cement_links_p3 = country_values(cement_links_p3)

efficiency1 = n.links.efficiency.loc[
    n.links.index.str.contains("Cement", case=False)
].iloc[0]
efficiency2 = n.links.efficiency2.loc[
    n.links.index.str.contains("Cement", case=False)
].iloc[0]
efficiency3 = n.links.efficiency3.loc[
    n.links.index.str.contains("Cement", case=False)
].iloc[0]
...
cement_process_cc_p0 = (
    n.links_t.p0.filter(like="cement CC", axis=1).sum() * timestep
)  # tCO2 entering CC

cement_process_cc_p1 = (
    n.links_t.p1.filter(like="cement CC", axis=1).sum() * timestep
)  # tCO2 to atmosphere
cement_process_cc_p1 = country_values(cement_process_cc_p1)

cement_process_cc_p2 = (
    n.links_t.p2.filter(like="cement CC", axis=1).sum() * timestep
)  # tCO2 to store
cement_process_cc_p2 = country_values(cement_process_cc_p2)

cement_process_cc_p3 = (
    n.links_t.p3.filter(like="cement CC", axis=1).sum() * timestep
)  # MWh heat required for carbon capture
cement_process_cc_p3 = country_values(cement_process_cc_p3)

efficiency1 = n.links.efficiency.loc[
    n.links.index.str.contains("cement CC", case=False)
].iloc[0]
efficiency2 = n.links.efficiency2.loc[
    n.links.index.str.contains("cement CC", case=False)
].iloc[0]
...
cement_process_p0 = (
    n.links_t.p0.filter(like="cement process emis to atmosphere", axis=1).sum()
    * timestep
)  # kton limestone per year per node
...
tot_cement_process = (
    -(cement_process_p1 + cement_process_cc_p1).fillna(cement_process_p1).sum() / 1e6
)  # Mton CO2 output of cement industry
...
share_noncc = cement_process_p1.sum() / (
    cement_process_p1.sum() + cement_process_cc_p1.sum()
)
...
steel_load = (
    n.loads_t.p.filter(like="steel", axis=1).sum() * timestep
)  # kton steel per year per node
steel_load = country_values(steel_load)

steel_supply_bof = (
    -n.links_t.p1.filter(like="BOF", axis=1).sum() * timestep
)  # kton steel per year per node
steel_supply_bof = country_values(steel_supply_bof)

steel_supply_eaf = (
    -n.links_t.p1.filter(like="DRI-EAF", axis=1).sum() * timestep
)  # kton steel per year per node
...
bof_links_p0 = (
    n.links_t.p0.filter(like="BOF", axis=1).sum() * timestep
)  # kton iron per year per node
bof_links_p0 = country_values(bof_links_p0)
bof_links_p2 = (
    n.links_t.p2.filter(like="BOF", axis=1).sum() * timestep
)  # kton coal per year per node
bof_links_p2 = country_values(bof_links_p2)
bof_links_p3 = (
    n.links_t.p3.filter(like="BOF", axis=1).sum() * timestep
)  # MWh industry heat per year per node
bof_links_p3 = country_values(bof_links_p3)
bof_links_p4 = (
    n.links_t.p4.filter(like="BOF", axis=1).sum() * timestep
)  # MWh electricity per year per node
bof_links_p4 = country_values(bof_links_p4)
bof_links_p5 = (
    n.links_t.p5.filter(like="BOF", axis=1).sum() * timestep
)  # tCO2 per year per node
bof_links_p5 = country_values(bof_links_p5)

efficiency1 = n.links.efficiency.loc[
    n.links.index.str.contains("BOF", case=False)
].iloc[0]
efficiency2 = n.links.efficiency2.loc[
    n.links.index.str.contains("BOF", case=False)
].iloc[0]
...
efficiency4 = n.links.efficiency4.loc[
    n.links.index.str.contains("BOF", case=False)
].iloc[0]
efficiency5 = n.links.efficiency5.loc[
    n.links.index.str.contains("BOF", case=False)
].iloc[0]
...
eaf_links_p0 = (
    n.links_t.p0.filter(like="EAF", axis=1).sum() * timestep
)  # kton iron per year per node
eaf_links_p0 = country_values(eaf_links_p0)
eaf_links_p2 = (
    n.links_t.p2.filter(like="EAF", axis=1).sum() * timestep
)  # kton coal per year per node
eaf_links_p2 = country_values(eaf_links_p2)
eaf_links_p3 = (
    n.links_t.p3.filter(like="EAF", axis=1).sum() * timestep
)  # MWh industry heat per year per node
eaf_links_p3 = country_values(eaf_links_p3)
eaf_links_p4 = (
    n.links_t.p4.filter(like="EAF", axis=1).sum() * timestep
)  # MWh electricity per year per node
eaf_links_p4 = country_values(eaf_links_p4)

efficiency1 = n.links.efficiency.loc[
    n.links.index.str.contains("EAF", case=False)
].iloc[0]
efficiency2 = n.links.efficiency2.loc[
    n.links.index.str.contains("EAF", case=False)
].iloc[0]
...
ng_efficiency = n.links.efficiency.loc[
    n.links.index.str.contains("CH4 to DRI", case=False)
].iloc[0]
h2_efficiency = n.links.efficiency.loc[
    n.links.index.str.contains("H2 to DRI", case=False)
].iloc[0]

# They should match with bus2 in EAF
check_dri = (
    ng_drigas.sum() * ng_efficiency
    + h2_drigas.sum() * h2_efficiency
    - eaf_links_p2.sum()
)

# Check if emissions balances are okay
dri_process_p0 = (
    n.links_t.p0.filter(like="steel dri process", axis=1).sum() * timestep
)  # kton limestone per year per node
dri_process_p0 = country_values(dri_process_p0)
dri_process_p1 = (
    n.links_t.p1.filter(like="steel dri process", axis=1).sum() * timestep
)  # kton limestone per year per node
dri_process_p1 = country_values(dri_process_p1)
dri_efficiency = n.links.efficiency.loc[
    n.links.index.str.contains("steel dri process", case=False)
].iloc[0]
...
# Check BOF emissions
bof_process_p0 = (
    n.links_t.p0.filter(like="steel bof process", axis=1).sum() * timestep
)  # kton limestone per year per node
bof_process_p0 = country_values(bof_process_p0)
bof_process_p1 = (
    n.links_t.p1.filter(like="steel bof process", axis=1).sum() * timestep
)  # kton limestone per year per node
bof_process_p1 = country_values(bof_process_p1)
bof_efficiency = n.links.efficiency.loc[
    n.links.index.str.contains("steel bof process", case=False)
].iloc[0]

check_bof_emis = bof_process_p0.sum() * bof_efficiency - bof_process_p1.sum()

bof_cc_p0 = (
    n.links_t.p0.filter(like="steel bof CC", axis=1).sum() * timestep
)  # kton limestone per year per node
bof_cc_p0 = country_values(bof_cc_p0)
bof_cc_p1 = (
    n.links_t.p1.filter(like="steel bof CC", axis=1).sum() * timestep
)  # kton limestone per year per node
bof_cc_p1 = country_values(bof_cc_p1)
bof_cc_p2 = (
    n.links_t.p2.filter(like="steel bof CC", axis=1).sum() * timestep
)  # kton limestone per year per node
bof_cc_p2 = country_values(bof_cc_p2)
bof_cc_efficiency1 = n.links.efficiency.loc[
    n.links.index.str.contains("steel bof CC", case=False)
].iloc[0]
bof_cc_efficiency2 = n.links.efficiency2.loc[
    n.links.index.str.contains("steel bof CC", case=False)
].iloc[0]
