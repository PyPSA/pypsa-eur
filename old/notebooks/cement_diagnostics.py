...
cement_dem = (
    n.loads_t.p.filter(like="cement", axis=1).sum().sum() * timestep
)  # kt cement
...
for year, n in n_dict.items():
    ...
    cement_dem = (
        n_dict[year].loads_t.p.filter(like="cement", axis=1).sum().sum() * timestep
    )  # Mt cement
    ...
    cement_tech_dict[year] = (
        cement_prod_per_tech  # Store cement production for this year
    )
