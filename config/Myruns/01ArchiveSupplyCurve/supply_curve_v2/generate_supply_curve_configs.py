"""
Generate supply curve V2 scenario configs for CDR credit prices €0–€500 in €50 steps.

Each priced config (S01-S10) is identical to S00 except:
- cdr_credit_prices_by_scope reflects the scenario price
...
S00 is the zero-price anchor with sequestration-time crediting (config.S00-seq-000eur.yaml).
...
# Supply curve V2 scenario: credit price = €{price}/tCO₂ for DAC and BECCS.
...
solving:
  ...
  agg_p_nom_limits:
    ...
    file: config/Myruns/supply_curve_v2/nuclear_p_nom_limits.csv
  constraints:
    CCL: true
    EQ: false
    BAU: false
...
prices = range(50, 501, 50)  # 50, 100, 150, ..., 500 (S00 at €0 already exists)
...
print("S00 (€0) = config.S00-seq-000eur.yaml (sequestration-time crediting, zero price).")
print("\nScenario summary:")
print(f"  S00: €0/t   (config.S00-seq-000eur.yaml)")
for index, price in enumerate(prices, start=1):
