...
def load_network(fn, tech_costs, config, combine_hydro_ps=True):
    ...
    if combine_hydro_ps:
        n.storage_units.loc[n.storage_units.carrier.isin(
            {'PHS', 'hydro'}), 'carrier'] = 'hydro+PHS'
    ...
    return pd.concat(
        [
            (n.storage_units["p_nom_opt"] *
             n.storage_units["max_hours"]).groupby(
                n.storage_units["carrier"]).sum(),
            n.stores["e_nom_opt"].groupby(
                n.stores.carrier).sum()])


def aggregate_p_curtailed(n):
    return pd.concat(
        [
            ((n.generators_t.p_max_pu.sum().multiply(
                n.generators.p_nom_opt) -
                n.generators_t.p.sum()) .groupby(
                n.generators.carrier).sum()),
            ((n.storage_units_t.inflow.sum() -
              n.storage_units_t.p.sum()) .groupby(
                n.storage_units.carrier).sum())])
...
def aggregate_costs(n, flatten=False, opts=None, existing_only=False):
    ...
    for c, (p_nom, p_attr) in zip(
        ...
        if not existing_only:
            p_nom += "_opt"
        costs[(c.list_name, 'capital')] = (c.df[p_nom] * \
               c.df.capital_cost).groupby(c.df.carrier).sum()
