...
if __name__ == "__main__":
    if "snakemake" not in globals():
        ...
        snakemake = mock_snakemake(
            "plot_weather_data_map", configfiles=["../../config/config.test.yaml"]
        )
    ...
    data = dict(
        irradiation=dict(
            label=r"Mean Direct Solar Irradiance [W/m$^2$]", cmap="Oranges"
        ),
    ...
    for k, v in data.items():
        ...
        ax.add_feature(
            cartopy.feature.COASTLINE.with_scale("50m"), linewidth=0.5, zorder=2
        )
        ax.add_feature(
            cartopy.feature.BORDERS.with_scale("50m"), linewidth=0.5, zorder=2
        )
