...
def eez(country_shapes):
    ...
    df = df.loc[df['ISO_3digit'].isin([get_country('alpha_3', alpha_2=c) for c in snakemake.config['countries']])]
    df['name'] = df['ISO_3digit'].map(lambda c: get_country('alpha_2', alpha_3=c))
