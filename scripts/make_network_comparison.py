...
def get_lines_by_country(network):
    ...
    lines = (
        ...
        .merge(network.buses[["country"]], left_on="bus1", right_index=True, 
               suffixes=("0", "1"))
    )

    # Aggregate parallel lines between same bus pairs
    lines = (
        lines.groupby(["bus0", "bus1", "v_nom", "country0", "country1"], 
                     observed=True)
...
def prepare_comparison_data(lines_compare_to, lines_release, countries, version):
    ...
    label_map = {
        "compare_to": f"Comparison ({version})",
        "release": "New release"
    }
    ...
    def to_long_format(metric):
        ...
        return (
            ...
            .assign(
                parameter=lambda df: df["parameter"].str.replace(col_prefix, "").map(label_map)
            )
        )
