...
def add_land_transport(n, costs):
    ...
    if options["endogenous_transport"]==False:
        if any([fuel_cell_share == None, electric_share == None, ice_share == None]):
            logger.warning(
                f"Exogenous land transport selected, but not all land transport shares are defined."
            )
    ...
    if options["endogenous_transport"]==True:
        if all([fuel_cell_share != None, electric_share != None, ice_share != None]):
            logger.warning(
                f"Endogenous land transport is activated, but land transport shares are exogenously defined for the three types of land transport. The exogenous shares are enforced."
            )
        elif any([fuel_cell_share != None, electric_share != None, ice_share != None]):
            logger.warning(
                f"Endogenous land transport is activated, but land transport shares are exogenously defined. The exogenous shares are enforced, limiting the endogenous calculation of optimal shares"
            )
    ...
    if electric_share != 0:
        ...
        if float(electric_share or 0) > 0:
            ...
            n.links.loc[ev_index, "p_nom_extendable"] = False

            # available EVs for land transport demand
            pnom = (p_set.divide(eff)).max()
            pnom = pnom.add_suffix(' land transport EV')
            n.links.loc[ev_index,'p_nom'] = pnom * electric_share
    ...
    if fuel_cell_share != 0:
        ...
        if float(fuel_cell_share or 0) > 0:
            pnom = (p_set.divide(eff)).max()
            pnom=pnom.add_suffix(' land transport fuel cell')
            fuel_cell_index = n.links.carrier=='land transport fuel cell'
            n.links.loc[fuel_cell_index, "p_nom_extendable"] = False
            n.links.loc[fuel_cell_index,'p_nom'] = fuel_cell_share * pnom
            n.links.loc[fuel_cell_index, "capital_cost"] = 0
    ...
    if ice_share != 0:
        ...
        if float(ice_share or 0) > 0:
            pnom = (p_set.divide(eff)).max()
            pnom = pnom.add_suffix(' land transport oil')
            ice_index = n.links.carrier=='land transport oil'
            n.links.loc[ice_index, "p_nom_extendable"] = False
            n.links.loc[ice_index ,'p_nom'] = pnom
            n.links.loc[ice_index, "capital_cost"] = 0


def adjust_land_transport(n):
    # Adjust p_min_pu and p_max_pu after temporal aggregation, because the temporal changing efficiencies 
    # lead to wrong calculations otherwise
