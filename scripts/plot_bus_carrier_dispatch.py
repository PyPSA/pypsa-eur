...
def get_bus_balance(n, bus_name):
    ...
    for link in n.links.index[n.links.bus1 == bus_name]:
        ...
        if carrier not in carriers:
            carriers[carrier] = 0.0
        carriers[carrier] += -n.links_t.p1[link]  # negative sign because bus1 is outflow
    ...
    for load in loads:
        ...
        if f"{carrier} load" not in carriers:
            carriers[f"{carrier} load"] = 0.0
        carriers[f"{carrier} load"] += -n.loads_t.p[load]  # negative sign for consumption
    ...
    for store in stores:
        ...
        carriers[f"{carrier} charge"] += -n.stores_t.p[store]  # negative sign for consumption
...
def create_dispatch_plot(bus_data, bus_name):
    ...
    plot_container = bokeh_column(children=[plot for plot in plots.values()], sizing_mode='stretch_width')
...
if __name__ == "__main__":
    ...
    if bus_data:
        ...
        # Save the plot
        save(layout)
        logger.info(f"Interactive carrier dispatch plot saved to {snakemake.output.html}")
    else:
