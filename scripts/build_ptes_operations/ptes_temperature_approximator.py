...
class PtesTemperatureApproximator:
    ...
    # dafür müsste dann aber auch die Berechnung der PTES Temperatur angepasst werden, wenn die aktiv ist. Sodass wir für die COP Berechnung 90°C top temperature haben

    def forward_temperature_boost_ratio(self) -> xr.DataArray:
        """
        Calculate the additional lift required between the store's max top temperature
        and the forward temperature to operate the store at maximum capacity. ???? Amos nochmal fragen
        Notes
        -----
        The total thermal output required to reach the forward temperature is:
