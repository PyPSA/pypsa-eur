...
def fun_update_elec_capacities(
    n, carriers_to_update, method_increase, nuts2_ES_file, dic_nuts_file
):
    def _update_in_network(df, required_capacity, method_increase):
        ...
        if int(required_capacity) > int(initial_capacity):
            ...
            if (method_increase == "proportional") and (initial_capacity == 0):
                logger.info(
                    f'########## [PyPSA-Spain] <add_electricity.py> WARNING: no initial capacity for carrier {cc} in {rr_name}. Using "additional" method'
                )
                capacity_added_at_each_bus = (
                    required_capacity - initial_capacity
                ) / df.shape[0]
    ...
    with open(dic_nuts_file) as archivo:
    ...
    for cc, ff in carriers_to_update.items():
        ...
        for rr in df_installed_capacity.columns:
            ...
            if rr in dic_nuts2.values():
                ...
                gdf_buses_local = gpd.sjoin(
                    gdf_buses, gdf_region, how="inner", predicate="within"
                )
                ...
                df_generators_local_cc = df_generators.loc[
                    (df_generators["bus"].isin(list_buses_local))
                    & (df_generators["carrier"] == cc)
                    & (df_generators["p_nom"] > 0.01)
                ]
                ...
                if len(df_generators_local_cc) == 0:
                    logger.info(
                        f"########## [PyPSA-Spain] <add_electricity.py> WARNING: No bus with generator {cc} in {rr_name}. Updating capacity was not possible."
                    )
                    ...
                    ############### Update capacities
                    df_updated = _update_in_network(
                        df_generators_local_cc, required_capacity, method_increase
                    )
                    ...
                    if df_updated["p_nom"].sum() > df_updated["p_nom_max"].sum():
                        sys.exit(
                            f"Not enough space to allocate required capacity of carrier {cc} in region {rr_name}"
                        )
                    ...
                    while (df_updated["p_nom"] > df_updated["p_nom_max"]).any():
                        # Take the first one
                        index_conflictive = df_updated[
                            df_generators_local_cc["p_nom"] > df_updated["p_nom_max"]
                        ].index[0]

                        # Identify overcapacity
                        overcapacity = (
                            df_updated.at[index_conflictive, "p_nom"]
                            - df_updated.at[index_conflictive, "p_nom_max"]
                        )
                        ...
                        df_to_sanitise = _update_in_network(
                            df_to_sanitise, required_capacity, method_increase
                        )
                    ...
                    if final_capacity == initial_capacity:
                        logger.info(
                            f"########## [PyPSA-Spain] <add_electricity.py> INFO: {cc} capacity matches in {rr_name}."
                        )

                    else:
                        logger.info(
                            f"########## [PyPSA-Spain] <add_electricity.py> INFO: {cc} capacity in {rr_name} was updated from {initial_capacity} to {final_capacity}."
                        )

                    saturated_nodes = (
                        df_updated["p_nom"] == df_updated["p_nom_max"]
                    ).sum()

                    if saturated_nodes > 0:
                        logger.info(
                            f"########## [PyPSA-Spain] <add_electricity.py> WARNING: Maximum capacity was reached in {saturated_nodes}!!"
                        )
...
if __name__ == "__main__":
    ...
    dic_nuts_file = (
        snakemake.input.dic_nuts
    )  # is employed to loop over the regions considered in PyPSA-Spain

    ##### call function
    if update_elec_capacities["enable"]:
        fun_update_elec_capacities(
            n, carriers_to_update, method_increase, nuts2_ES_file, dic_nuts_file
        )
    #
    #
