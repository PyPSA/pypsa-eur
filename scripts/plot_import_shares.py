...
if __name__ == "__main__":
    ...
    eb = eb.unstack(1).groupby(lambda x: "import" if "import" in x or "external" in x else x).sum()
    ...
    imp_mix = pd.DataFrame(imp_mix.where(imp_mix > 1e-3).dropna().rename(index=CARRIERS).sort_values(ascending=False))
    ...
    imp_mix.T.plot.barh(ax=ax_mix, stacked=True, legend=True, color=[tech_colors[COLOR_MAPPING[i]]for i in imp_mix.index])
    ...
    secax_mix = ax_mix.secondary_xaxis('top', functions=(lambda x: x / total_imp * 100, lambda x: x * total_imp * 100))
