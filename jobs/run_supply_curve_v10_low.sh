#!/bin/bash
# =============================================================================
# run_supply_curve_v10_low.sh
# Submit one LSF job per supply curve scenario — v10 LOW B-bracket (loB, ×0.52 net-heat).
# Results go to: results/supply_curve_v10/supply_curve_v10_low/<scenario>/
#
# Usage (from repo root):
#   bash jobs/run_supply_curve_v10_low.sh           # submit all scenarios
#   bash jobs/run_supply_curve_v10_low.sh S01 S03   # submit specific scenarios by prefix
#
# Flags:
...
CONFIG_DIR="$REPO_ROOT/config/Myruns/supply_curve_v10/supply_curve_v10_low"
LOG_DIR="$REPO_ROOT/logs/supply_curve_v10_low"
mkdir -p "$LOG_DIR"
...
echo ""
echo "Submitting ${#CONFIGS[@]} supply curve scenario(s) [v10 LOW B-bracket (loB, ×0.52 net-heat)]..."
echo ""

for cfg in "${CONFIGS[@]}"; do
    basename_cfg=$(basename "$cfg" .yaml)
    scenario_id="${basename_cfg#config.}"
    job_name="v10l-${scenario_id}"
...
echo "Host:     \$(hostname)"
echo "Scenario: ${scenario_id} [v10 LOW B-bracket (loB, ×0.52 net-heat)]"
echo "Config:   ${cfg}"
echo "Started:  \$(date)"
...
echo "  bjobs -J 'v10l-*'"
echo ""
echo "Results: results/supply_curve_v10/supply_curve_v10_low/"
echo "Logs:    $LOG_DIR"
