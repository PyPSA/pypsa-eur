#!/bin/bash
# =============================================================================
# run_supply_curve_high.sh
# Submit one LSF job per supply curve scenario (S0 through S10) — HIGH cost.
...
#   bash jobs/run_supply_curve_high.sh           # submit all scenarios
#   bash jobs/run_supply_curve_high.sh S01 S03   # submit specific scenarios by prefix
#
# Flags:
...
CONFIG_DIR="$REPO_ROOT/config/Myruns/supply_curve_high"
LOG_DIR="$REPO_ROOT/logs/supply_curve_high"
mkdir -p "$LOG_DIR"
...
echo ""
echo "Submitting ${#CONFIGS[@]} supply curve scenario(s) [HIGH cost]..."
echo ""

for cfg in "${CONFIGS[@]}"; do
    basename_cfg=$(basename "$cfg" .yaml)               # e.g. config.S01-050eur
    scenario_id="${basename_cfg#config.}"               # e.g. S01-050eur
    job_name="hi-${scenario_id}"                        # e.g. hi-S01-050eur
...
echo "Host:     \$(hostname)"
echo "Scenario: ${scenario_id} [HIGH cost]"
echo "Config:   ${cfg}"
echo "Started:  \$(date)"
