#!/bin/bash
# =============================================================================
# run_supply_curve_v9_medium.sh
# Submit one LSF job per supply curve scenario — v9 MEDIUM cost.
# Results go to: results/supply_curve_v9/supply_curve_v9_medium/<scenario>/
#
# Usage (from repo root):
#   bash jobs/run_supply_curve_v9_medium.sh           # submit all scenarios
#   bash jobs/run_supply_curve_v9_medium.sh S01 S03   # submit specific scenarios by prefix
#
# Flags:
...
CONFIG_DIR="$REPO_ROOT/config/Myruns/supply_curve_v9/supply_curve_v9_medium"
LOG_DIR="$REPO_ROOT/logs/supply_curve_v9_medium"
mkdir -p "$LOG_DIR"
...
echo ""
echo "Submitting ${#CONFIGS[@]} supply curve scenario(s) [v9 MEDIUM cost]..."
echo ""

for cfg in "${CONFIGS[@]}"; do
    basename_cfg=$(basename "$cfg" .yaml)
    scenario_id="${basename_cfg#config.}"
    job_name="v9m-${scenario_id}"
...
echo "Host:     \$(hostname)"
echo "Scenario: ${scenario_id} [v9 MEDIUM cost]"
echo "Config:   ${cfg}"
echo "Started:  \$(date)"
...
echo "  bjobs -J 'v9m-*'"
echo ""
echo "Results: results/supply_curve_v9/supply_curve_v9_medium/"
echo "Logs:    $LOG_DIR"
