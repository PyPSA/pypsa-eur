#!/bin/bash
# =============================================================================
# run_supply_curve_v9_high.sh
# Submit one LSF job per supply curve scenario — v9 HIGH cost.
# Results go to: results/supply_curve_v9/supply_curve_v9_high/<scenario>/
#
# Usage (from repo root):
#   bash jobs/run_supply_curve_v9_high.sh           # submit all scenarios
#   bash jobs/run_supply_curve_v9_high.sh S01 S03   # submit specific scenarios by prefix
#
# Flags:
...
CONFIG_DIR="$REPO_ROOT/config/Myruns/supply_curve_v9/supply_curve_v9_high"
LOG_DIR="$REPO_ROOT/logs/supply_curve_v9_high"
mkdir -p "$LOG_DIR"
...
echo ""
echo "Submitting ${#CONFIGS[@]} supply curve scenario(s) [v9 HIGH cost]..."
echo ""

for cfg in "${CONFIGS[@]}"; do
    basename_cfg=$(basename "$cfg" .yaml)
    scenario_id="${basename_cfg#config.}"
    job_name="v9hi-${scenario_id}"
...
echo "Host:     \$(hostname)"
echo "Scenario: ${scenario_id} [v9 HIGH cost]"
echo "Config:   ${cfg}"
echo "Started:  \$(date)"
...
echo "  bjobs -J 'v9hi-*'"
echo ""
echo "Results: results/supply_curve_v9/supply_curve_v9_high/"
echo "Logs:    $LOG_DIR"
