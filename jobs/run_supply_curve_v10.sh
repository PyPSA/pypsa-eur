#!/bin/bash
# =============================================================================
# run_supply_curve_v10.sh
# Submit one LSF job per supply curve scenario — v10 medium B-bracket (hoB, ×0.52 net-heat).
# Results go to: results/supply_curve_v10/<scenario>/
#
# Usage (from repo root):
#   bash jobs/run_supply_curve_v10.sh           # submit all scenarios
#   bash jobs/run_supply_curve_v10.sh S01 S03   # submit specific scenarios by prefix
#
# Flags:
...
CONFIG_DIR="$REPO_ROOT/config/Myruns/supply_curve_v10"
LOG_DIR="$REPO_ROOT/logs/supply_curve_v10"
mkdir -p "$LOG_DIR"
...
echo ""
echo "Submitting ${#CONFIGS[@]} supply curve scenario(s) [v10 medium B-bracket (hoB, ×0.52 net-heat)]..."
echo ""

for cfg in "${CONFIGS[@]}"; do
    basename_cfg=$(basename "$cfg" .yaml)
    scenario_id="${basename_cfg#config.}"
    job_name="v10-${scenario_id}"
...
echo "Host:     \$(hostname)"
echo "Scenario: ${scenario_id} [v10 medium B-bracket (hoB, ×0.52 net-heat)]"
echo "Config:   ${cfg}"
echo "Started:  \$(date)"
...
echo "  bjobs -J 'v10-*'"
echo ""
echo "Results: results/supply_curve_v10/"
echo "Logs:    $LOG_DIR"
