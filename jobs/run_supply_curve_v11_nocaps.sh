#!/bin/bash
# =============================================================================
# run_supply_curve_v11_nocaps.sh
# Submit one LSF job per supply curve scenario — v11 medium (no CCS growth caps).
# Results go to: results/supply_curve_v11_nocaps/<scenario>/
#
# Usage (from repo root):
#   bash jobs/run_supply_curve_v11_nocaps.sh           # submit all scenarios
#   bash jobs/run_supply_curve_v11_nocaps.sh S01 S03   # submit specific scenarios by prefix
#
# Flags:
...
CONFIG_DIR="$REPO_ROOT/config/Myruns/supply_curve_v11_nocaps"
LOG_DIR="$REPO_ROOT/logs/supply_curve_v11_nocaps"
mkdir -p "$LOG_DIR"
...
echo ""
echo "Submitting ${#CONFIGS[@]} supply curve scenario(s) [v11 medium (no CCS growth caps)]..."
echo ""

for cfg in "${CONFIGS[@]}"; do
    basename_cfg=$(basename "$cfg" .yaml)
    scenario_id="${basename_cfg#config.}"
    job_name="v11nc-${scenario_id}"
...
echo "Host:     \$(hostname)"
echo "Scenario: ${scenario_id} [v11 medium (no CCS growth caps)]"
echo "Config:   ${cfg}"
echo "Started:  \$(date)"
...
echo "  bjobs -J 'v11nc-*'"
echo ""
echo "Results: results/supply_curve_v11_nocaps/"
echo "Logs:    $LOG_DIR"
