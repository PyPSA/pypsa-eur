...
n_dict = {year: pypsa.Network(res_directory + "base_s_39_lvopt___" + str(year) + ".nc") for year in years}
...
for year, n in n_dict.items():
    ...
    steel_dem = n_dict[year].loads_t.p.filter(like='steel', axis=1).sum() * timestep  # Mt steel
    ...
    steel_prod_per_country.index = steel_prod_per_country.index.str[:2]  # Keep only the first two characters (country code)
    steel_prod_per_country = steel_prod_per_country.groupby(steel_prod_per_country.index).sum()  # Group by country code
    steel_prod_dict[year] = steel_prod_per_country  # Store steel production for this year
    
    steel_prod_per_tech.index = steel_prod_per_tech.index.str.extract('(EAF|BOF)', expand=False)
    steel_prod_per_tech = steel_prod_per_tech.groupby(steel_prod_per_tech.index).sum()  # Group by country code
...
ax.scatter(bar_positions, steel_dem_df.iloc[0], color='black', zorder=3, label='Steel Demand')
