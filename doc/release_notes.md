...
* Removed the unused rule `build_simplified_population_layouts`, which pointed to a script whose inputs it did not provide and whose output no rule consumed.
* Documentation: the *Rules Overview* pages are generated from the workflow. Every rule in `rules/*.smk` now carries a one-line docstring, which Snakemake also prints as the job message in place of the removed `message:` blocks, and the `rules()` macro in `doc/macros.py` renders per rule its summary, script, inputs, outputs, settings and wildcards parsed from the Snakemake files (`doc/rule_docs.py`), followed by the script's module docstring. The former *Building Electricity Networks* and *Building Sector-Coupled Networks* pages are split into pages under `doc/rules/` by theme: regions and weather, transmission grid, electricity demand and supply, population and energy balances, heating, transport, industry, costs and prices, biomass, gas and carbon, composing and solving, plotting. Legacy input, output and settings listings in module docstrings are removed in favour of the generated blocks. Missing and minimal module docstrings were written following a compact template recorded in `CLAUDE.md` and the contributing guide; `test/test_docs.py` checks that every rule is documented.
...
* The cost database for retrofitting of the thermal envelope of buildings has been updated. Now, for calculating the space heat savings of a building, losses by thermal bridges and ventilation are included as well as heat gains (internal and by solar radiation). See the section [retro](rules/heating.md#rule-build_retro_cost) for more details on the retrofitting module.
