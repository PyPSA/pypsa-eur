...
<!-- Upcoming Release -->
<!-- ================= -->
* The `plotting:` configuration block is now covered by the Pydantic config validation, like all other top-level config sections. `config/plotting.default.yaml` is generated from `scripts/lib/validation/config/plotting.py`; run `pixi run generate-config` after editing it. No default values change in effect, aside from a new `heat_source_map` section and numeric `balance_map`/`balance_map_interactive` settings now rendering as floats.
