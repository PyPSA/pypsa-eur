...
* Added an option to alter the capital cost of carriers by a factor via ``carrier+factor`` in the ``{opts}`` wildcard. This can be usefule for exploring uncertain cost parameters. Example: ``solar+0.5`` reduces the capital cost of solar to 50\% of original values (`#TBD <https://github.com/PyPSA/pypsa-eur/pull/TBD>`_).
