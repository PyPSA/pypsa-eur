...
For running perfect foresight scenarios, set the foresight mode in your
configuration (see `config/test/config.perfect.yaml` for a complete example):
...
2. For the first horizon, insert the assets built before the base year via
   functions from [add_existing_baseyear][] inside [compose_network][].
...
- [compose_network][]
    ...
    **Existing capacities** (handled by functions from [add_existing_baseyear][]
    called in [compose_network][]):

    Existing conventional generators are retrieved from the [powerplants.csv](https://pypsa-eur.readthedocs.io/en/latest/preparation/#powerplants)
